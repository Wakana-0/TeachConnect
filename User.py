...
def play_notification_sound():
    try:
        sound = pygame.mixer.Sound('sound.mp3')  # 替换为你的音频文件路径
        sound.play()
...
# 设置存储路径
APP_DATA_PATH = os.path.join(os.getenv("APPDATA"), "TCC")
...
IP_STORAGE_FILE = os.path.join(CACHE_PATH, "recent_ips.json")
NAME_STORAGE_FILE = os.path.join(CACHE_PATH, "recent_names.json")
USER_CREDENTIALS_FILE = os.path.join(USER_DATA_PATH, "users.json")
...
class MessagingApp(QWidget):
    ...
    def send_message(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "发送失败", f"发送失败: {e}")
